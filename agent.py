...
def extract_keywords_with_jieba(text, product_name="", product_category="", top_n=15):
    ...
    try:
        ...
        for word in words:
            ...
            # 过滤条件：2-6字、中文或英文、不是停用词、不是产品名
            if (2 <= len(word) <= 6 and 
                re.match(r'^[\u4e00-\u9fa5a-zA-Z]+$', word) and
                word not in stopwords and
                word not in product_words):
                word_count[word] += 1
...
with tab1:
    ...
    user_request = st.text_area("📝 输入你要调研的产品",
        placeholder="例如：iPhone 15 Pro（高端手机）、蜜雪冰城柠檬水（茶饮品类）、科沃斯T20 Pro（扫地机器人）、三养火鸡面（方便食品）...",
        height=80)
